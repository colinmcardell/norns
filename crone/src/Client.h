...
template <int NumIns, int NumOuts> class Client {
  ...
  public:
    virtual void setup() {
        ...
        for (int i = 0; i < NumIns; ++i) {
            ...
            inPort[i] = jack_port_register(client, name, JACK_DEFAULT_AUDIO_TYPE, JackPortIsInput, 0);
            if (inPort[i] == nullptr) {
        ...
        for (int i = 0; i < NumOuts; ++i) {
            ...
            os << "output_" << (i + 1);
            const char *name = os.str().c_str();
            outPort[i] = jack_port_register(client, name, JACK_DEFAULT_AUDIO_TYPE, JackPortIsOutput, 0);
  ...
  public:
    ...
    template <int N, int M> bool connect(Client<N, M> *other, int sinkIdx, int sourceIdx) {
