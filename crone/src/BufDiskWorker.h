...
class BufDiskWorker {
  ...
  private:
    ...
    static void copyLoop(float *dst, const float *src, size_t frDur, size_t frFadeTime, float preserve, float x,
                         float phi, Step &&step);
  ...
  public:
    ...
    static void requestClearWithFade(size_t idx, float start = 0, float dur = -1, float fadeTime = 0,
                                     float preserve = 0);

    static void requestCopy(size_t srcIdx, size_t dstIdx, float srcStart = 0, float dstStart = 0, float dur = -1,
                            float fadeTime = 0, float preserve = 0, bool reverse = false);

    // read mono soundfile to mono buffer
    static void requestReadMono(size_t idx, std::string path, float startSrc = 0, float startDst = 0, float dur = -1,
                                int chanSrc = 0, float preserve = 0, float mix = 1);

    // read and de-interleave stereo soundfile to 2x mono buffers
    static void requestReadStereo(size_t idx0, size_t idx1, std::string path, float startSrc = 0, float startDst = 0,
  ...
  private:
    ...
    static void clearBufferWithFade(BufDesc &buf, float start = 0, float dur = -1, float fadeTime = 0,
                                    float preserve = 0);

    static void copyBuffer(BufDesc &buf0, BufDesc &buf1, float srcStart = 0, float dstStart = 0, float dur = -1,
                           float fadeTime = 0, float preserve = 0, bool reverse = false);

    static void readBufferMono(const std::string &path, BufDesc &buf, float startSrc = 0, float startDst = 0,
                               float dur = -1, int chanSrc = 0, float preserve = 0, float mix = 1) noexcept;

    static void readBufferStereo(const std::string &path, BufDesc &buf0, BufDesc &buf1, float startSrc = 0,
                                 float startDst = 0, float dur = -1, float preserve = 0, float mix = 1) noexcept;

    static void writeBufferMono(const std::string &path, BufDesc &buf, float start = 0, float dur = -1) noexcept;

    static void writeBufferStereo(const std::string &path, BufDesc &buf0, BufDesc &buf1, float start = 0,
