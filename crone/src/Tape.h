...
template <int NumChannels> class Tape {
  private:
  ...
  public:
    ...
    class SfStream {
      protected:
        ...
        typedef enum { Starting, Playing, Stopping, Stopped } EnvState;
      ...
      public:
        SfStream() : file(nullptr), status(0), isRunning(false), shouldStop(false) {
        ...
        virtual // from any thread
            ...
            if (isRunning) {
                ...
                this->th = std::make_unique<std::thread>([this]() { this->diskLoop(); });
                this->th->detach();
            }
        }
    ...
    class Writer : public SfStream {
      ...
      public:
            ...
            while (!SfStream::shouldStop) {
                {
                    ...
                    this->cv.wait(lock, [this] { return this->dataPending; });
        ...
        Writer() : SfStream(), dataPending(false), numFramesCaptured(0), maxFrames(JACK_MAX_FRAMES) {
        }
    }; // Writer class
      ...
      public:
        ...
        bool open(const std::string &path) {
            ...
            if ((this->file = sf_open(path.c_str(), SFM_READ, &sfInfo)) == NULL) {
                ...
                std::cout << "Tape Reader:: cannot open sndfile" << path << " for output (" << errstr << ")"
                          << std::endl;
                return false;
            }
      ...
      private:
        ...
        void diskLoop() override {
            ...
            while (!SfStream::shouldStop) {
                {
                    std::unique_lock<std::mutex> lock(this->mut);
                    this->cv.wait(lock, [this] { return this->needsData; });
                    // check for spurious wakeup
                    if (!needsData) {
                        continue;
                    }
