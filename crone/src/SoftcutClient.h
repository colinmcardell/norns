...
class SoftcutClient : public Client<2, 2> {
  ...
  public:
    ...
    void readBufferMono(const std::string &path, float startTimeSrc = 0.f, float startTimeDst = 0.f, float dur = -1.f,
                        int chanSrc = 0, int chanDst = 0, float preserve = 0.f, float mix = 1.f) {
        BufDiskWorker::requestReadMono(bufIdx[chanDst], path, startTimeSrc, startTimeDst, dur, chanSrc, preserve, mix);
    }

    void readBufferStereo(const std::string &path, float startTimeSrc = 0.f, float startTimeDst = 0.f, float dur = -1.f,
    ...
    void copyBuffer(int srcChan, int dstChan, float srcStart = 0.f, float dstStart = 0.f, float dur = -1,
                    float fadeTime = 0.f, float preserve = 0.f, bool reverse = false) {
        if (srcChan < 0 || srcChan > 1 || dstChan < 0 || dstChan > 1) {
            return;
        }
        BufDiskWorker::requestCopy(bufIdx[srcChan], bufIdx[dstChan], srcStart, dstStart, dur, fadeTime, preserve,
                                   reverse);
    }
