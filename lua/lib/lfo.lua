...
local lfo_rates_as_strings = { "1/16", "1/8", "1/4", "5/16", "1/3", "3/8", "1/2", "3/4", "1", "1.5", "2", "3", "4", "6",
...
function LFO:add(args)
  return self.new(args.shape, args.min, args.max, args.depth, args.mode, args.period, args.action, args.phase,
    args.baseline)
end
    ...
    if _lfo.depth > 0 then
      ...
      elseif _lfo.shape == 'random' then
        ...
        if prev_value ~= _lfo.rand_value then
          rand_value = util.linlin(min, max, _lfo.scaled_min, _lfo.scaled_max,
            math.random(math.floor(min * 100), math.floor(max * 100)) / 100)
...
function LFO:add_params(id, sep, group)
  if id ~= nil then
    if params.lookup["lfo_" .. id] == nil then
      ...
      params:set_action("lfo_shape_" .. id,
        function(x) self:set('shape', params:lookup_param("lfo_shape_" .. id).options[x]) end)

      params:add_number("lfo_depth_" .. id, "lfo depth", 0, 100, self:get('depth') * 100,
      ...
      params:add_number('lfo_phase_' .. id, 'lfo phase', 0, 100, self:get('phase') * 100,
        function(param) return (param:get() .. '%') end)
      params:set_action('lfo_phase_' .. id, function(x)
        self:set('phase', x / 100)
      end)

      params:add_number('lfo_offset_' .. id, 'lfo offset', -100, 100, self:get('offset') * 100,
        function(param) return (param:get() .. "%") end)
      ...
      params:add_option("lfo_baseline_" .. id, "lfo baseline", baseline_options,
        tab.key(baseline_options, 'from ' .. self:get('baseline')))
      ...
      local current_period_as_rate = self:get('mode') == 'clocked' and
      lfo_rates[lfo_rates_as_strings[self:get('period')]] or self:get('period')
      ...
      params:add_option("lfo_reset_target_" .. id, "reset lfo to", reset_destinations,
        tab.key(reset_destinations, self:get('reset_target')))
