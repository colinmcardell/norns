...
function TL.hotswap(old, new)
    if type(old) == 'table' then
        ...
        elseif s.is_sequins(old) then
            old:settable(new)                           -- sequins
        else                                            -- regular table
...
local doact = function(fn)
    ...
    if type(fn) == 'string' then
        return fn                          -- strings are keywords
    elseif isfn(fn) then
        return fn()                        -- call it directly
...
function TL:_launch(q)
    self.lq = q; return self
end
