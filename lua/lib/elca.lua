...
function CA:neighbors(i)
   ...
   if i <= self.bound_l then
      if self.bound_mode_l == CA.BOUND_LOW then
         l = 0
      elseif self.bound_mode_l == CA.BOUND_HIGH then
         l = 1
      else
         l = self.state[self.bound_r]
      end
   else
      l = self.state[i - 1]
   end
   ...
   if i >= CA.NUM_STATES then
      if self.bound_mode_r == CA.BOUND_LOW then
         r = 0
      elseif self.bound_mode_r == CA.BOUND_HIGH then
         r = 1
      else
         r = self.state[self.bound_l]
      end
   else
      r = self.state[i + 1]
   end
