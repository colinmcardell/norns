...
int open_spi() {
    ...
    int outcome =
        0 || (ioctl(fd, SPI_IOC_WR_MODE, &mode) < 0) || (ioctl(fd, SPI_IOC_WR_BITS_PER_WORD, &bits_per_word) < 0) ||
        (ioctl(fd, SPI_IOC_WR_MAX_SPEED_HZ, &speed_hz) < 0) || (ioctl(fd, SPI_IOC_WR_LSB_FIRST, &little_endian) < 0);
    if (outcome != 0) {
...
void ssd1322_set_gamma(double g) {
    ...
    write_command_with_data(SSD1322_SET_GRAYSCALE_TABLE, // GS0 is skipped.
                            gs[0x1], gs[0x2], gs[0x3], gs[0x4], gs[0x5], gs[0x6], gs[0x7], gs[0x8], gs[0x9], gs[0xA],
